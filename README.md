...
- Runtime settings, recovery markers, and logs are fixed to `D:\monitor_sleep_v0.01.01`.
...
1. Download `monitor_sleep_v0.01.01_windows_x64.exe` and `SHA256SUMS.txt` from the release page.
2. Create `D:\monitor_sleep_v0.01.01` and place the executable in that directory.
3. Verify the downloaded executable against `SHA256SUMS.txt`.
4. Open PowerShell as Administrator and run:

```powershell
D:\monitor_sleep_v0.01.01\monitor_sleep_v0.01.01_windows_x64.exe --install
```

Check the service:

```powershell
D:\monitor_sleep_v0.01.01\monitor_sleep_v0.01.01_windows_x64.exe --status
```
...
```powershell
cd D:\monitor_sleep_v0.01.01
.\monitor_sleep_v0.01.01_windows_x64.exe --tray
```
...
- Run `.\monitor_sleep_v0.01.01_windows_x64.exe --tray` again to restore the tray agent after selecting `끝내기`.
...
```powershell
D:\monitor_sleep_v0.01.01\monitor_sleep_v0.01.01_windows_x64.exe --uninstall
```

When upgrading from an earlier version, uninstall the old service with its executable before installing `v0.01.01`.
